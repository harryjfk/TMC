    ...
    public partial class UCPropertyTempControl : UCTemperatureControl
    {
      ...
      protected override void HandleMover(MouseEventArgs e)
        ...
        internal void AddMaterial(DSRD.T_MaterialRow t_MaterialRow)
        {
            ...
            {
                if (frmadd.Position == -1)
                {
                    Algorithm.Materiales.Add(t_MaterialRow);
                    Algorithm.Spesor.Add((double)frmadd.Espesor / 1000);
                    TempList.Add(new PropertyTempItem(this, TempList, t_MaterialRow, /*frmadd.Formato*/null) { BeginValue = 0, EndValue = frmadd.Espesor, });
                    CalculateInvalidate(Algorithm.Materiales.Count - 1);
                }
                else
                    if(frmadd.Position == 0)
                    {
        ...
        internal void LoadFromProject()
        {
            ...
            if (Algorithm.Project!=null)
            {
                Algorithm.Project.tempamb = ((PropertyItemList) TempList).TemperaturaAmbiente;
                Algorithm.Project.tempdise = ((PropertyItemList)TempList).TemperaturaInterna;
            }
            ReCalculateAll();
        }
    }
        ...
        public int TemperaturaInterna
        {
            ...
            set
            {
                ...
                if(_control!=null)
                {
                    ...
                    if (((UCPropertyTempControl)_control).Algorithm.Project!=null)
                    ((UCPropertyTempControl)_control).Algorithm.Project.tempdise = value; 
                }
        ...
        public int TemperaturaAmbiente
        {
            ...
            set
            {
                _tempamb = value; if (_control != null)
                {
                    ((UCPropertyTempControl)_control).ReCalculateAll();
                    if (((UCPropertyTempControl)_control).Algorithm.Project != null)
                    ((UCPropertyTempControl)_control).Algorithm.Project.tempamb = value;   
                }
