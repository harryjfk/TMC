...
namespace TMC.Components.TemperatureControl
{
    public class SIARAlgorithm
    {
                ...
                case 1:
                    {
                        return
                            double.Parse(
                                (1.842*
                                 (Math.Pow(
                                     ((List<double>) LastSisParcial["TEXP_E"])[Materiales.Count] , 0.55))).ToString());
        ...
        internal DSRD.P_ReporteComposicionDataTable GetComposicion()
        {
            TDataSet.P_ReporteComposicion.Clear();
            for (int index = 0; index < Materiales.Count; index++)
            {
                DSRD.T_MaterialRow materialRow = Materiales[index];
                var ts = ((List<double>)LastSisParcial["TMTP"])[index+1];

                var t = TDataSet.P_ReporteComposicion.NewP_ReporteComposicionRow();
                t.Tecnologia = Project.nameproject;
                t.Material = materialRow.namematerial;
                t.Espesor = (Spesor[index]*1000).ToString();
                t.Volumen = (Spesor[index] * Project.area*1000).ToString();
                t.Costo = materialRow.GetT_Propiedad_ProduccionRows()[0].getCostoProm().ToString();
                t.Estres = decimal.Round( decimal.Parse( (materialRow.Estres(Spesor[index],
                                              (double) LastSisParcial["QPAR"],
                                              ((List<double>)
                                               LastSisParcial["LP"])[index+1], ts).ToString())),2).ToString();
                t.TMedia = decimal.Round(decimal.Parse(ts.ToString()),2).ToString();
                var l = new Dictionary<String, String>();
                l.Add("T", ts.ToString());
                t.Cond = decimal.Round(decimal.Parse((new Ecuation(materialRow.GetFormula("K"), "", l).RightSide.Parameters.AsDouble()).ToString()), 2).ToString();
           
                TDataSet.P_ReporteComposicion.AddP_ReporteComposicionRow(t);
            }
            return TDataSet.P_ReporteComposicion;
        }
        public double Espesor
        {
            get { return Spesor.Sum() * 1000; }
        ...
        public double Volumen
        {
            get
            {
                return Espesor * Project.area;
            }

        }
        public double GetSpesor(DSRD.T_MaterialRow material)
        {
            var i = Materiales.IndexOf(material);
            if (i != -1)
                return Spesor[i] * 1000;
            return -1;
        }
        public double Peso
        {
            get
            {
                return Materiales.Sum(material => GetSpesor(material) * material.GetT_Propiedad_FisicaRows()[0].Densidad * Project.area);
            }
        }
        ...
        public double Costo
        {
            ...
            get
            {
                return Materiales.Sum(t => t.GetT_Propiedad_ProduccionRows()[0].getCostoProm());
        ...
        public double PerdidaTotal
        {
            get
            {
                if (LastSisParcial != null)
                    return (double)LastSisParcial["QPAR"] * Project.area;
                return 0;
            }
        ...
        public double FlujoCalorico
        {
            ...
            {
                if (LastSisParcial != null)
                    return (double)LastSisParcial["QPAR"]; return 0;
            }

        }
        public double TempExterna
        {
            get
            {
                if (LastSisParcial != null)
                {
                    var t = (List<double>)LastSisParcial["TEXP"];
                    return t[t.Count - 1];
                }
                else
                    return 0;
        ...
        public DSRD.P_ReporteDiseñoDataTable GetDiseño()
        {
           TDataSet.P_ReporteDiseño.Clear();
            var t = TDataSet.P_ReporteDiseño.NewP_ReporteDiseñoRow();
            t.Tecnologia = Project.nameproject;
            t.Volumen = decimal.Round(decimal.Parse(Volumen.ToString()),2).ToString();
            t.TempExtern = decimal.Round(decimal.Parse(TempExterna.ToString()),2).ToString();
            t.Peso = decimal.Round(decimal.Parse((Peso/1000).ToString()),2).ToString();
            t.Perdida = decimal.Round(decimal.Parse(PerdidaTotal.ToString()),2).ToString();
            t.Flujo = decimal.Round(decimal.Parse(FlujoCalorico.ToString()),2).ToString();
            t.Espesor = decimal.Round(decimal.Parse(Espesor.ToString()),2).ToString();
            t.Area = decimal.Round(decimal.Parse(Project.area.ToString()),2).ToString();

            TDataSet.P_ReporteDiseño.AddP_ReporteDiseñoRow(t);
            return TDataSet.P_ReporteDiseño;
        }
    }
}
