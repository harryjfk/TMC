...
namespace TMC.MCC
{
    ...
    {
        partial class P_ReporteTecnologiaDataTable
        {
        ...
        */
        ...
        partial class T_PalanquillaRow
        {
            ...
            public double Area_Intercambio(String longitud)
            {
        ...
        {
            public void AddRow(T_ParametroValorRow row)
            {
                var tableadpaer = new DSMCCTableAdapters.T_ParametroValorTableAdapter();
                tableadpaer.Insert(idtecnologia, row.idparametro, row.valor, row.um, false);
            }
            ...
            public void Reporte(int? types,bool fill=true)
            {
                if (fill)
                {
                ...
                frmreport.ucReport1.LoadFromFile(Application.StartupPath + @"\Reportes\MCC\RepTecnologia.repx", ((DSMCC)Table.DataSet).P_ReporteTecnologia, null);
        ...
        partial class T_ParametrosDataTable
        {
            public int FindByName(String name)
            {
                ...
                for (int i =0;i<Rows.Count;i++)
                    if(  ((T_ParametrosRow)Rows[i]).nomparametro.ToLower() == name.ToLower())
        ...
        partial class T_MaquinaRow
        {
            ...
            /// <param name="Valores"></param>
            /// <returns></returns>
            public List<double> CalorTotalTransferido(DSMCC.T_PalanquillaRow palanquillaRow, string longitud, Dictionary<string, double> Valores)
            {
            ...
            /// <param name="Valores"></param>
            /// <returns></returns>
            public List<double> CantAguaMax_EP(DSMCC.T_PalanquillaRow palanquillaRow, string longitud, Dictionary<string, double> Valores)
            {
            ...
            /// <param name="Valores"></param>
            /// <returns></returns>
            public double VelocidadAgua_EP(DSMCC.T_PalanquillaRow palanquillaRow, string longitud, Dictionary<string, double> Valores)
            {
            ...
            /// <param name="Valores"></param>
            /// <returns></returns>
            public double EspesorCapa(double diametro, double altura, int idpalanquilla, DSMCC.T_PalanquillaRow palanquillaRow, string longitud, Dictionary<string, double> Valores)
            {
            ...
            {
                var t = new T_ParametroValorTableAdapter();
                t.Fill(((DSMCC)Table.DataSet).T_ParametroValor);
            ...
            public T_TecnologiaRow Calculate(int idmaquina, int idacero, int idpalanquilla, int longitud, int codigo)
            {
                ...
                var tableadpater = new DSMCCTableAdapters.T_TecnologiaTableAdapter();
                tableadpater.Update(((DSMCC) Table.DataSet).T_Tecnologia);
                ...
                result.AddRow(result.AddParameter("lo", decimal.Round((decimal) LargoMetalurgico(), 6), "m"));
                result.AddRow(result.AddParameter("vc", decimal.Round((decimal) VelocidadColada(diametro, 0.6), 6),
                                                  "Kg/min"));
                result.AddRow(result.AddParameter("ve",
                                                  decimal.Round(
                                                      (decimal) Velocidad_Extraccion(diametro, 0.6, idpalanquilla), 6),
                                                  "m"));
                result.AddRow(result.AddParameter("vp",
                                                  decimal.Round(
                                                      (decimal) Velocidad_Produccion(diametro, 0.6, idpalanquilla), 6),
                                                  "lineas/min"));
                result.AddRow(result.AddParameter("tipac",
                                                  decimal.Round(
                                                      (decimal)
                                                      PermanenciaCasuelayDistribuidor(diametro, 0.6, idpalanquilla, 75,
                                                                                      4), 6), "min"));
                result.AddRow(result.AddParameter("tipad",
                                                  decimal.Round(
                                                      (decimal)
                                                      PermanenciaCasuelayDistribuidor(diametro, 0.6, idpalanquilla, 10,
                                                                                      4), 6), "min"));
                result.AddRow(result.AddParameter("lmcal",
                                                  decimal.Round(
                                                      (decimal) LongitudMetCalc(diametro, 0.6, idpalanquilla, 28.5), 6),
                                                  "m"));
                result.AddRow(result.AddParameter("ctt",
                                                  decimal.Round(
                                                      (decimal)
                                                      CalorTotalTransferido(palanquilla, palanquilla.tipopalanquilla,
                                                                            temp)[2], 6), "Kcal/min"));
                result.AddRow(result.AddParameter("cmae",
                                                  decimal.Round(
                                                      (decimal)
                                                      CantAguaMax_EP(palanquilla, palanquilla.tipopalanquilla, temp)[2],
                                                      6), "L/min"));
                result.AddRow(result.AddParameter("vma",
                                                  decimal.Round(
                                                      (decimal)
                                                      VelocidadAgua_EP(palanquilla, palanquilla.tipopalanquilla, temp),
                                                      6), "m/s"));
                result.AddRow(result.AddParameter("ep",
                                                  decimal.Round(
                                                      (decimal)
                                                      EspesorCapa(diametro, 0.6, idpalanquilla, palanquilla,
                                                                  palanquilla.tipopalanquilla, temp), 6), "m"));
                result.AddRow(result.AddParameter("om",
                                                  decimal.Round(
                                                      (decimal) OscilacionMolde(diametro, 0.6, idpalanquilla), 6), "opm"));
                result.AddRow(result.AddParameter("ts",
                                                  decimal.Round((decimal) TiempoSanado(diametro, 0.6, idpalanquilla), 6),
                                                  "s"));
                result.AddRow(result.AddParameter("dmo",
                                                  decimal.Round(
                                                      (decimal) DistanciaMarcaOscilacion(diametro, 0.6, idpalanquilla),
                                                      6), "mm"));
                result.AddRow(result.AddParameter("esz1",
                                                  decimal.Round(
                                                      (decimal) EnfriamientoSecZ1(diametro, 0.6, idpalanquilla), 6),
                                                  "l/min"));
                result.AddRow(result.AddParameter("esz2",
                                                  decimal.Round(
                                                      (decimal) EnfriamientoSecZ2(diametro, 0.6, idpalanquilla), 6),
                                                  "l/min"));
                result.AddRow(result.AddParameter("esz3",
                                                  decimal.Round(
                                                      (decimal) EnfriamientoSecZ3(diametro, 0.6, idpalanquilla), 6),
                                                  "l/min"));
                result.AddRow(result.AddParameter("al", decimal.Round((decimal) AceiteLubricacion(), 6), "ml/min"));
            ...
            }public string NoteCient(string srt)
            {
                ...
                if (num < 0)
                {
                    ...
                    return ((Math.Pow(10, count) * num).ToString().Substring(0, 5) + "x10-" + count);
                }
                else
                {
                    for (int i = 0; i < s.Length; i++)
                    ...
                    return ((num / Math.Pow(10, count - 1)).ToString().Substring(0,5) + "x10+" + (count - 1)).ToString();
                }
